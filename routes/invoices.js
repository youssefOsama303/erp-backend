...
router.get("/", [checkPermission('accounting', 'read')], async (req, res) => {
...
router.get("/:id", [checkPermission('accounting', 'read')], async (req, res) => {
  try {
    const { rows } = await pool.query(
...
router.post("/", [checkPermission('accounting', 'write')], async (req, res) => {
...
router.patch("/:id/status", [checkPermission('accounting', 'write')], async (req, res) => {
...
router.delete("/:id", [checkPermission('accounting', 'write')], async (req, res) => {
  try {
    const { rows } = await pool.query(
    ...
    if (!rows[0]) return res.status(404).json({ message: "الفاتورة غير موجودة" });
    res.json({ message: "تم حذف الفاتورة بنجاح", invoice: rows[0] });
  } catch (err) {
    res.status(500).json({ message: err.message });
  }
});
